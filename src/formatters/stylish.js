...
const format = (data) => {
  const iter = (innerData, depth = 1) => {
    const res = innerData.map((item) => {
      ...
      switch (status) {
        case 'removed':
          return `${getIndent(depth + 2)}- ${name}: ${stringify(value, depth + 3)}`;
        case 'added':
          return `${getIndent(depth + 2)}+ ${name}: ${stringify(value, depth + 3)}`;
        case 'unModified':
          return `${getIndent(depth + 2)}  ${name}: ${stringify(value, depth + 2)}`;
        case 'modified':
          return `${getIndent(depth + 2)}- ${name}: ${stringify(valueRemoved, depth + 3)}\n${getIndent(depth + 2)}+ ${name}: ${stringify(valueAdded, depth + 3)}`;
        case 'parentNode':
          return `${getIndent(depth) + 2}  ${name}: ${format(child, depth + 2)}`;
        default:
